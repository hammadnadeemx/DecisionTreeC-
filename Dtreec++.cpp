...
public:
	Car(){	}
	void adddetails(string input){ // need to break up string into parts
...
public:
	...
	carlist(const carlist& rhs){ // copyconstructor
		if (this != &rhs) {
			length=rhs.length;
			size=rhs.size;
	...
	void InsertItem(Car item){ // copy items
		if (IsFull()){
			Car *ptr = new Car[size + 5];
			for (int i = 0; i<size; i++){
				ptr[i] = dataarray[i];
	...
	Car getitem(int index){ // array like access (copy not alias)
		Car temp;
	...
	void copylistattributes(int column, int quality, carlist& input){// breaks list into sub parts column is atribute and quality is type ( high low med etc)
		this->MakeEmpty();
		...
		if (run != 0){
			...
			case 1: //buying
				...
				else{ cout << " out of function bounds buying\n"; }
				break;
			case 2:// maint
				if (quality == 1){// low med high vhigh (1,2,3,4)
					for (int i = 0; i<run; i++){
				...
				else{ cout << " out of function bounds maintiance \n"; }
				break;
			case 3:// doors
				if (quality == 1){// two three four 5more (1,2,3,4)
					for (int i = 0; i<run; i++){
				...
				else if (quality == 3){// two three four 5more (1,2,3,4)
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].doors == "4")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else if (quality == 4){// two three four 5more (1,2,3,4)
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].doors == "5more")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else{ cout << " out of function bounds doors\n"; }
				break;
			case 4:// persons
				if (quality == 1){// two four more 1,2,3
					for (int i = 0; i<run; i++){
				...
				else if (quality == 3){// two four more 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].persons == "more")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else{ cout << " out of function bounds persons\n"; }
				break;
			case 5:// luggage
				if (quality == 1){// small med big 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].lug_boot == "small")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else if (quality == 2){// small med big 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].lug_boot == "med")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else if (quality == 3){// small med big 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].lug_boot == "big")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else{ cout << " out of function bounds luggage\n"; }
				break;
			case 6:// safety
				if (quality == 1){// low med high 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].safety == "low")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else if (quality == 2){// low med high 1,2,3
					for (int i = 0; i<run; i++){
						if (input.dataarray[i].safety == "med")
							this->InsertItem(input.dataarray[i]);
					}
				}
				else if (quality == 3){// low med high 1,2,3
					for (int i = 0; i<run; i++){
	...
	float findentrophy(){// calculates total entrophy wrt outcome
	...
	carlist& operator=(const carlist &rhs) {// Only do assignment if RHS is a different object from this.
		if (this != &rhs) {
			delete [] dataarray;
			length=rhs.length;
			size=rhs.size;
	...
	~carlist(){// had to comment this out due to segmentation fault errors (should have stuck to pointers)
		delete [] dataarray;
	}
};
struct dtnode{
...
private:
	dtnode *root;
	void destroysubtree(dtnode *&nodePtr){// set this leaf to null before calling this function to prevent segmentation fault errors
	...
	bool checkindividualcase(int x, Car list){ // 1-6 for atributes
		switch (x){
		case 1:
	...
	void countoutcomes(int& unaccept, int& accept, int& good, int& vgood, carlist& input){// counts all outputs used at node divisions
	...
	int findmaximumgain(carlist& input, Car list){// returns a int (1-6 and zero for pureclass and 7 for covering all attributes) which denotes a quality that best divides data avoids already bucketed terms
		if (input.checkpuresubset(input))
			return 0;
			...
			for (int i = 1; i<7; i++){// 1-6 attributes
				if (!checkindividualcase(i, list)){ // push each into own list and calulate new entrophys and number of terms
					switch (i){
					case 1: //buying
	...
	void constructtree(carlist input, dtnode*& node, Car list){// recursive dtree builder base case ?
	...
	int determineindex(string input, int row){ // row is atribute and input is what the car object has stored. returns a index to which child of the node to go to next
		switch (row){
		case 1:
...
public:
	...
	void builddtree(carlist& input){// build decision tree for a given list of cars
	...
	string getresult(Car input){// traverses the tree and tells if car is acceptable/not/good/vgood
		string temp, div;
		int index;
		dtnode *ptr = root;
		while (1){
			...
			if (div == "buying"){
				...
				for (int i = 0; i<4; i++){ //find max
					if (max<array[i]){
						max = array[i];
						slot = i;
					}
				}
				for (int i = 0; i<4; i++){ //find duplicate cases only designed for 2 equal entities (eg acc==unacc)
